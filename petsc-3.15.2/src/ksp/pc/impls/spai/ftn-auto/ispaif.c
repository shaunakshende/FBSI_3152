...
extern "C" {
#endif
PETSC_EXTERN void  pcspaisetepsilon_(PC pc,double *epsilon1, int *__ierr)
{
*__ierr = PCSPAISetEpsilon(
	(PC)PetscToPointer((pc) ),*epsilon1);
}
PETSC_EXTERN void  pcspaisetnbsteps_(PC pc,int *nbsteps1, int *__ierr)
{
*__ierr = PCSPAISetNBSteps(
	(PC)PetscToPointer((pc) ),*nbsteps1);
}
PETSC_EXTERN void  pcspaisetmax_(PC pc,int *max1, int *__ierr)
{
*__ierr = PCSPAISetMax(
	(PC)PetscToPointer((pc) ),*max1);
}
PETSC_EXTERN void  pcspaisetmaxnew_(PC pc,int *maxnew1, int *__ierr)
{
*__ierr = PCSPAISetMaxNew(
	(PC)PetscToPointer((pc) ),*maxnew1);
}
PETSC_EXTERN void  pcspaisetblocksize_(PC pc,int *block_size1, int *__ierr)
{
*__ierr = PCSPAISetBlockSize(
	(PC)PetscToPointer((pc) ),*block_size1);
}
PETSC_EXTERN void  pcspaisetcachesize_(PC pc,int *cache_size, int *__ierr)
{
*__ierr = PCSPAISetCacheSize(
	(PC)PetscToPointer((pc) ),*cache_size);
}
PETSC_EXTERN void  pcspaisetverbose_(PC pc,int *verbose, int *__ierr)
{
*__ierr = PCSPAISetVerbose(
	(PC)PetscToPointer((pc) ),*verbose);
}
PETSC_EXTERN void  pcspaisetsp_(PC pc,int *sp, int *__ierr)
{
*__ierr = PCSPAISetSp(
