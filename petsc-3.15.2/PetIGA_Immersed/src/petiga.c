...
PetscErrorCode IGACreate(MPI_Comm comm,IGA *_iga)
{
  ...
  *_iga = iga;
  ...
  for (i=0; i<3; i++) {
    ...
    ierr = IGAInterfaceCreate(&iga->interface[i][0]);CHKERRQ(ierr);
    ierr = IGAInterfaceCreate(&iga->interface[i][1]);CHKERRQ(ierr);
    iga->proc_sizes[i] = -1;
  }
...
PetscErrorCode IGADestroy(IGA *_iga)
{
  ...
  for (i=0; i<3; i++) {
    ...
    ierr = IGAInterfaceDestroy(&iga->interface[i][0]);CHKERRQ(ierr);
    ierr = IGAInterfaceDestroy(&iga->interface[i][1]);CHKERRQ(ierr);
  }
...
#undef  __FUNCT__
#define __FUNCT__ "IGAGetInterface"
/*@
   IGAGetInterface - Returns a pointer to a specific side of the i^th
   interface associated with the IGA.

   Not Collective

   Input Parameters:
+  iga - the IGA context
.  i - the interface index
-  side - the side index: 0 (left) or 1 (right)
...
.keywords: IGA, boundary
@*/
PetscErrorCode IGAGetInterface(IGA iga,PetscInt i,PetscInt side,IGAInterface *interface)
{
  PetscInt       dim;
  PetscErrorCode ierr;
  PetscFunctionBegin;
  PetscValidHeaderSpecific(iga,IGA_CLASSID,1);
  PetscValidPointer(interface,4);
  dim = (iga->dim > 0) ? iga->dim : 3;
  if (i < 0)    SETERRQ1(((PetscObject)iga)->comm,PETSC_ERR_ARG_OUTOFRANGE,"Index %D must be nonnegative",i);
  if (i >= dim) SETERRQ2(((PetscObject)iga)->comm,PETSC_ERR_ARG_OUTOFRANGE,"Index %D, but dimension %D",i,iga->dim);
  if (iga->dof <= 0) SETERRQ(((PetscObject)iga)->comm,PETSC_ERR_ARG_WRONGSTATE,"Must call IGASetDof() first");
  if (side < 0) side = 0; /* XXX error ?*/
  if (side > 1) side = 1; /* XXX error ?*/
  if (iga->interface[i][side]->dof != iga->dof) {
    ierr = IGAInterfaceInit(iga->interface[i][side],iga->dof);CHKERRQ(ierr);
  }
  *interface = iga->interface[i][side];
  PetscFunctionReturn(0);
}
...
PetscErrorCode IGASetUp(IGA iga)
{
  ...
  for (i=iga->dim; i<3; i++) {
    ...
    ierr = IGAInterfaceReset(iga->interface[i][0]);CHKERRQ(ierr);
    ierr = IGAInterfaceReset(iga->interface[i][1]);CHKERRQ(ierr);
