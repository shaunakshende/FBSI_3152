...
exec='../ex47'
testname='runex47_2d_p1p1_mg_r1'
label='ts_tutorials-ex47_2d_p1p1_mg_r1'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex47_2d_p1p1_mg_r1.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ts/tutorials/output/ex47_2d_p1p1_mg_r1.out ex47_2d_p1p1_mg_r1.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
