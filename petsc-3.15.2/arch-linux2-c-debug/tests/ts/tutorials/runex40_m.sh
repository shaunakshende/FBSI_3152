...
for insize in ${nsize_in}; do
   for its_exact_final_time in ${ts_exact_final_time_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ts/tutorials/output/ex40_m.out ex40_m.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+ts_exact_final_time-${its_exact_final_time} ""
      else
