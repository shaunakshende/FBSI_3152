...
exec='../ex56'
testname='runex56_hpddm'
label='snes_tutorials-ex56_hpddm'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex56_hpddm.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex56_hpddm.out ex56_hpddm.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
