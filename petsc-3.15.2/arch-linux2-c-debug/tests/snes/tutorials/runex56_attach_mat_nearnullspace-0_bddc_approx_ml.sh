...
for insize in ${nsize_in}; do
   ...
   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex56_attach_mat_nearnullspace-0_bddc_approx_ml.out ex56_attach_mat_nearnullspace-0_bddc_approx_ml.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
