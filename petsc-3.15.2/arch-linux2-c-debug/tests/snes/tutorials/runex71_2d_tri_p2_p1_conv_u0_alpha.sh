...
exec='../ex71'
testname='runex71_2d_tri_p2_p1_conv_u0_alpha'
label='snes_tutorials-ex71_2d_tri_p2_p1_conv_u0_alpha'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex71_2d_tri_p2_p1_conv_u0_alpha.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex71_2d_tri_p2_p1_conv_u0_alpha.out ex71_2d_tri_p2_p1_conv_u0_alpha.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
