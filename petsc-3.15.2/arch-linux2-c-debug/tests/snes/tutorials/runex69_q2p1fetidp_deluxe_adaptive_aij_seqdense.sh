...
exec='../ex69'
testname='runex69_q2p1fetidp_deluxe_adaptive_aij_seqdense'
label='snes_tutorials-ex69_q2p1fetidp_deluxe_adaptive_aij_seqdense'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex69_q2p1fetidp_deluxe_adaptive_aij_seqdense.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex69_q2p1fetidp_deluxe_adaptive.out ex69_q2p1fetidp_deluxe_adaptive_aij_seqdense.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
