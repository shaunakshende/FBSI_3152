...
exec='../ex13'
testname='runex13_2d_p1_fas_full'
label='snes_tutorials-ex13_2d_p1_fas_full'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex13_2d_p1_fas_full.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex13_2d_p1_fas_full.out ex13_2d_p1_fas_full.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
