...
exec='../ex12'
testname='runex12_quad_hpddm_reuse_threshold'
label='snes_tutorials-ex12_quad_hpddm_reuse_threshold'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex12_quad_hpddm_reuse_threshold.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex12_quad_hpddm_reuse_threshold.out ex12_quad_hpddm_reuse_threshold.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
