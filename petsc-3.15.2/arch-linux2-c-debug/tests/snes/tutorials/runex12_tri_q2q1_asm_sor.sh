...
exec='../ex12'
testname='runex12_tri_q2q1_asm_sor'
label='snes_tutorials-ex12_tri_q2q1_asm_sor'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex12_tri_q2q1_asm_sor.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex12_tri_q2q1_asm_sor.out ex12_tri_q2q1_asm_sor.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
