...
exec='../ex12'
testname='runex12_fas_newton_0'
label='snes_tutorials-ex12_fas_newton_0'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex12_fas_newton_0.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/snes/tutorials/output/ex12_fas_newton_0.out ex12_fas_newton_0.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
