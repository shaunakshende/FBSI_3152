...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex1_18.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/vec/vec/utils/tagger/tutorials/output/ex1_18.out ex1_18.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
