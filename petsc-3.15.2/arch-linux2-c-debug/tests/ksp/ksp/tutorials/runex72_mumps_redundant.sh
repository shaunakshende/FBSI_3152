...
for insize in ${nsize_in}; do
   for ipc_redundant_number in ${pc_redundant_number_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex72_mumps_redundant.out ex72_mumps_redundant.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+pc_redundant_number-${ipc_redundant_number} ""
      else
