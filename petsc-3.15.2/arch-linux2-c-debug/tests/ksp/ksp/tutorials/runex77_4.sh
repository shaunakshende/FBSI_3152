...
exec='../ex77'
testname='runex77_4'
label='ksp_ksp_tutorials-ex77_4'
runfiles=''
...
for insize in ${nsize_in}; do
   for imat_type in ${mat_type_in}; do
      ...
      petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args}  -mat_type ${imat_type}" ex77_4.tmp ${testname}.err "${label}+mat_type-${imat_type}" 
      res=$?

      if test $res = 0; then
         petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex77_4.out ex77_4.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+mat_type-${imat_type} ""
      else
