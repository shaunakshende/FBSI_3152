...
exec='../ex75f'
testname='runex75f_2_seq'
label='ksp_ksp_tutorials-ex75f_2_seq'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex75f_2_seq.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex75_2.out ex75f_2_seq.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
