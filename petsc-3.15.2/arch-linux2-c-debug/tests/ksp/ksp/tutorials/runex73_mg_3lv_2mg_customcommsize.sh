...
exec='../ex73'
testname='runex73_mg_3lv_2mg_customcommsize'
label='ksp_ksp_tutorials-ex73_mg_3lv_2mg_customcommsize'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex73_mg_3lv_2mg_customcommsize.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex73_mg_3lv_2mg_customcommsize.out ex73_mg_3lv_2mg_customcommsize.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
