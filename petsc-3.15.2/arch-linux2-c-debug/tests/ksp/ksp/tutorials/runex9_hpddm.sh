...
for insize in ${nsize_in}; do
   for iksp_type in ${ksp_type_in}; do
      for is2_ksp_type in ${s2_ksp_type_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex9_hpddm.out ex9_hpddm.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+ksp_type-${iksp_type}_s2_ksp_type-${is2_ksp_type} ""
         else
