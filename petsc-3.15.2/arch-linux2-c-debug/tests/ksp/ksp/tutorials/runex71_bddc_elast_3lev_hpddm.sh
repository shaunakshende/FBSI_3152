...
exec='../ex71'
testname='runex71_bddc_elast_3lev_hpddm'
label='ksp_ksp_tutorials-ex71_bddc_elast_3lev_hpddm'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex71_bddc_elast_3lev_hpddm.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex71_bddc_elast_3lev_hpddm.out ex71_bddc_elast_3lev_hpddm.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
