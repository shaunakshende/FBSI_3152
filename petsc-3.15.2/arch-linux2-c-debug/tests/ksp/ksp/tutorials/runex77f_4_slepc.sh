...
exec='../ex77f'
testname='runex77f_4_slepc'
label='ksp_ksp_tutorials-ex77f_4_slepc'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex77f_4_slepc.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex77_4.out ex77f_4_slepc.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
