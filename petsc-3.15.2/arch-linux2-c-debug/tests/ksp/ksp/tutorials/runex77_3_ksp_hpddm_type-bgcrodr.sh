...
for insize in ${nsize_in}; do
   for imat_type in ${mat_type_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tutorials/output/ex77_3_ksp_hpddm_type-bgcrodr.out ex77_3_ksp_hpddm_type-bgcrodr.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+mat_type-${imat_type} ""
      else
