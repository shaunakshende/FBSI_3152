...
for insize in ${nsize_in}; do
   for iksp_type in ${ksp_type_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/ksp/ksp/tests/output/ex6_4_ksp_norm_type-unpreconditioned_pc_type-none.out ex6_4_ksp_norm_type-unpreconditioned_pc_type-none.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+ksp_type-${iksp_type} ""
      else
