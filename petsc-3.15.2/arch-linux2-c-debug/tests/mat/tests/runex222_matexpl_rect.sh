...
for insize in ${nsize_in}; do
   for iexpl_type in ${expl_type_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/mat/tests/output/ex222_null.out ex222_matexpl_rect.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}expl_type-${iexpl_type} ""
      else
