...
for insize in ${nsize_in}; do
   for iK in ${K_in}; do
      for ilocal in ${local_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/mat/tests/output/ex70_1.out ex70_7.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+K-${iK}_local-${ilocal} ""
         else
