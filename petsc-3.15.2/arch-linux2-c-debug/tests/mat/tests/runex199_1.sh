...
for insize in ${nsize_in}; do
   for imat_coloring_type in ${mat_coloring_type_in}; do
      for imat_coloring_distance in ${mat_coloring_distance_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/mat/tests/output/ex199_1.out ex199_1.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+mat_coloring_type-${imat_coloring_type}_mat_coloring_distance-${imat_coloring_distance} ""
         else
