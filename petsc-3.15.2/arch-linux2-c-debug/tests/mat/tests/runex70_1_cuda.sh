         ...
         for ibgpu in ${bgpu_in}; do
            for iA_mat_type in ${A_mat_type_in}; do
               for itestshellops in ${testshellops_in}; do
                  ...
                  if test $res = 0; then
                     petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/mat/tests/output/ex70_1.out ex70_1_cuda.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+local-${ilocal}_xgpu-${ixgpu}_bgpu-${ibgpu}_A_mat_type-${iA_mat_type}_testshellops-${itestshellops} ""
                  else
