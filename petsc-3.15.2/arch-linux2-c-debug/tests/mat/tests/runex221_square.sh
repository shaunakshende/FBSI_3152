      ...
      for ikeep in ${keep_in}; do
         for isubmat in ${submat_in}; do
            for itest_axpy_different in ${test_axpy_different_in}; do
               ...
               if test $res = 0; then
                  petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/mat/tests/output/ex221_1.out ex221_square.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}rows_only-${irows_only}_keep-${ikeep}_submat-${isubmat}_test_axpy_different-${itest_axpy_different} ""
               else
