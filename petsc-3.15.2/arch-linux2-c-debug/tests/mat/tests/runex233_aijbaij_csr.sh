...
for insize in ${nsize_in}; do
   for imat_type in ${mat_type_in}; do
      for irect in ${rect_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/mat/tests/output/ex233_aijbaij_csr.out ex233_aijbaij_csr.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+mat_type-${imat_type}_rect-${irect} ""
         else
