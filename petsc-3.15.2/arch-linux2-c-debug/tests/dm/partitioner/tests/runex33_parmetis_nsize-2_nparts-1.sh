...
for insize in ${nsize_in}; do
   for ipwgts in ${pwgts_in}; do
      for ivwgts in ${vwgts_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/dm/partitioner/tests/output/ex33_parmetis_nsize-2_nparts-1.out ex33_parmetis_nsize-2_nparts-1.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+pwgts-${ipwgts}_vwgts-${ivwgts} ""
         else
