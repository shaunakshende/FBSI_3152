...
for insize in ${nsize_in}; do
   for imsh in ${msh_in}; do
      for iorder in ${order_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/dm/impls/plex/tests/output/ex99_B3.out ex99_B3.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+msh-${imsh}_order-${iorder} ""
         else
