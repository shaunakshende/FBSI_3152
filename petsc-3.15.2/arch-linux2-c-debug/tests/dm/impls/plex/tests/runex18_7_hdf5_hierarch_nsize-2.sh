...
exec='../ex18'
testname='runex18_7_hdf5_hierarch_nsize-2'
label='dm_impls_plex_tests-ex18_7_hdf5_hierarch_nsize-2'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex18_7_hdf5_hierarch_nsize-2.tmp ${testname}.err "${label}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/dm/impls/plex/tests/output/ex18_7_hdf5_hierarch_nsize-2.out ex18_7_hdf5_hierarch_nsize-2.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
   else
