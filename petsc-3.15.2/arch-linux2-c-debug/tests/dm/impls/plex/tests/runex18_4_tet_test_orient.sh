...
for insize in ${nsize_in}; do
   for irotate_interface_0 in ${rotate_interface_0_in}; do
      for irotate_interface_1 in ${rotate_interface_1_in}; do
         ...
         if test $res = 0; then
            petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/dm/impls/plex/tests/output/ex18_4_tet_test_orient.out ex18_4_tet_test_orient.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+rotate_interface_0-${irotate_interface_0}_rotate_interface_1-${irotate_interface_1} ""
         else
