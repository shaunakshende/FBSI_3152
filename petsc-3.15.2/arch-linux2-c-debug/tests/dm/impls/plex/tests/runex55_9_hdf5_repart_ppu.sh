...
exec='../ex55'
testname='runex55_9_hdf5_repart_ppu'
label='dm_impls_plex_tests-ex55_9_hdf5_repart_ppu'
runfiles=''
...
for insize in ${nsize_in}; do
   ...
   petsc_testrun "${mpiexec} -n ${insize} ${exec} ${args} " ex55_9_hdf5_repart_ppu.tmp ${testname}.err "${label}+nsize-${insize}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/dm/impls/plex/tests/output/ex55_9_hdf5_repart_ppu.out ex55_9_hdf5_repart_ppu.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize} ""
   else
