...
for insize in ${nsize_in}; do
   for ifaces in ${faces_in}; do
      ...
      if test $res = 0; then
         petsc_testrun "${diff_exe} /home/shaunak/Desktop/petsc-3.15.2/src/dm/impls/plex/tests/output/ex18_6_int_tet.out ex18_6_int_tet.tmp" diff-${testname}.out diff-${testname}.out diff-${label}+nsize-${insize}faces-${ifaces} ""
      else
